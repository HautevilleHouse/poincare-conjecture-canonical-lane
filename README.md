...
## Canonical Lane (defined term): the manifold-constrained local-to-global closure architecture (`PC1–PC8`)

Canonical Lane research workspace for the Millennium Problem:
proving that every simply connected, closed 3-manifold is homeomorphic to the 3-sphere via Ricci-flow continuation, rigidity, and geometrization closure.

## Main Manuscript

- [paper/POINCARE_CONJECTURE_PREPRINT.md](paper/POINCARE_CONJECTURE_PREPRINT.md)
- [paper/CANONICAL_ROUTING_INDEX.md](paper/CANONICAL_ROUTING_INDEX.md)

## Structure

- `paper/`
  - `POINCARE_CONJECTURE_PREPRINT.md`
...
1. Theorem chain first: read `paper/POINCARE_CONJECTURE_PREPRINT.md`.
...
Current PC runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/pc_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/pc_closure_guard.py`.
...
- Manuscript target: [paper/POINCARE_CONJECTURE_PREPRINT.md](paper/POINCARE_CONJECTURE_PREPRINT.md)
